...
public class UserInfoESServiceImpl implements IUserInfoESService {
    ...
    public void init() throws IOException, InvocationTargetException, IllegalAccessException {
        ...
        R<List<UserInfo>> result = remoteUserInfoService.list("inner");
        if (result == null || R.isError(result)) {
            throw new IOException("读取用户数据失败: " + (result == null ? "远程服务无响应" : result.getMsg()));
        }
        List<UserInfo> data = result.getData();
        if (data == null) {
            throw new IOException("读取用户数据失败: 远程服务返回空数据");
        }
        ...
        if (data.isEmpty()) {
            System.out.println("用户数据为空，已创建空索引");
            return;
        }
        ...
        for (UserInfo userInfo : data) {
            ...
            BeanUtils.copyProperties(es, userInfo);
