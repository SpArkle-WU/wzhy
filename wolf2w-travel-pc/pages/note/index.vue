      ...
      <el-tabs v-model="tabActive" class="blog-tabs" @tab-change="tabClick">
        <el-tab-pane label="最新游记" name="first">
          ...
          <div class="travelnotes-list">
            <NuxtLink v-for="item in newBlogList" :key="item.id" :to="'/note/details/' + item.id" target="_blank"
              ...
              <img class="travelnotes-item-img" :src="item.coverUrl || 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=%E6%97%85%E6%B8%B8%E6%97%A5%E8%AE%B0%E5%B0%81%E9%9D%A2%E6%B5%B7%E6%8A%A5%E5%9B%BE%E7%89%87%E4%BA%BA%E5%83%8F%E5%9C%A8%E8%87%AA%E7%84%B6%E9%A3%8E%E6%99%AF%E4%B8%AD%E5%BE%80%E8%B5%B0%E6%97%85%E8%A1%8C&image_size=landscape_16_9'" />
              <div class="travelnotes-info">
                <h3 class="travelnotes-info-title">{{ item.title || '（无标题）' }}</h3>
                <p class="travelnotes-info-desc">{{ item.summary || '（暂无概要）' }}</p>
                <div class="travelnotes-extra">
                  <div class="travelnotes-extra-left">
                    <div class="travelnotes-extra-place">
                      <i></i>
                      {{ item.destName || '未指定目的地' }}，by
                    </div>
                    <div class="travelnotes-extra-user">
                      <img v-if="item.author && item.author.headImgUrl" :src="item.author.headImgUrl" />
                      <img v-else src="" alt="" />
                      {{ item.author && item.author.nickname ? item.author.nickname : '匿名用户' }}
                    </div>
                    <div class="travelnotes-extra-nums">
                      <i></i>
                      {{ item.viewnum ?? 0 }}/{{ item.favornum ?? 0 }}
                    </div>
                  </div>
            ...
            <div v-if="newBlogList.length === 0" class="empty-tip">
              暂无最新游记，<NuxtLink to="/note/form" class="empty-btn">立即写一篇</NuxtLink>
            </div>
          </div>
        ...
        <el-tab-pane label="最热游记" name="second">
          ...
          <div class="travelnotes-list">
            ...
            <div v-if="hotBlogList.length === 0" class="empty-tip">
              暂无最热游记，<NuxtLink to="/note/form" class="empty-btn">立即写一篇</NuxtLink>
            </div>
          </div>
